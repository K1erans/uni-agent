...
import { applyEvent, awaitingApproval, emptyTranscript, foldTranscript, type Transcript, type TurnItem } from './transcript';
...
describe('Transcript', () => {
  ...
  it('replaces only the streaming turn and text, keeping the others identical', () => {
    const before = [
      ...
      at(chunk('m:0', 'first')),
      at(chunk('m:1', 'str')),
    ].reduce(applyEvent, emptyTranscript);
...
function onlyTurn(state: Transcript): TurnItem {
